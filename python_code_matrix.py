...
class geometric_brownian_motion_simulation(simulator):
    ...
    def calculate(self):
        ...
        all_paths = np.exp((self.drift_term - 0.5 * self.sigma**2 ) * dt + self.sigma * np.random.normal(0, np.sqrt(dt), size = (self.increments,self.simulations )))
        all_paths = all_paths.cumprod(0)
        # By definition, standard brownian motion starts at point initial_price
        all_paths = np.insert(all_paths, 0 , self.initial_price, 0)
        return all_paths
...
class standard_brownian_motion_simulation(simulator):
    ...
    def calculate(self):
        ...
        # Generate a random step in the y direction and update position
        all_paths = np.random.normal(0, np.sqrt(dt), size = (self.increments,self.simulations))
        all_paths = all_paths.cumsum(0)
        # By definition, standard brownian motion starts at point 0
        all_paths = np.insert(all_paths, 0 , 0, 0)
        return all_paths
